...
def check_conversations_for_replies():
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            # Find conversations that might need replies
            # - Last message was from recipient
            # - No reply in last 30 minutes
            # - Less than 5 messages in last 24 hours (rate limiting)
            cur.execute("""
                SELECT 
...
def schedule_dm_reply(conversation_id: int, account_id: int, recipient: str):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            # Don't reply if we've sent more than 3 messages in the last hour
