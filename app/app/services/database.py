...
def add_user(username, password):
    ...
    c = conn.cursor()
    try:
        c.execute("INSERT INTO app.users (username, password) VALUES (%s, %s)", (username, password))
        conn.commit()
        return True
...
def verify_user(username, password):
    conn = connect()
    c = conn.cursor()
    c.execute("SELECT password FROM app.users WHERE username = %s", (username,))
    row = c.fetchone()
    conn.close()
...
def get_user_id(username):
    conn = connect()
    c = conn.cursor()
    c.execute("SELECT id FROM app.users WHERE username = %s", (username,))
    row = c.fetchone()
    conn.close()
